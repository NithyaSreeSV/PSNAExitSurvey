...
<table class="table table-striped table-bordered table-hover">
        <thead>
            <tr><th style='text-align:center'>PO.No</th><th><center>Program Outcomes</center><th style='text-align:center'>Exit Survey</th></tr>
...
// Fetch PO ID and corresponding averages from po_avg
$avg_query = "SELECT postt.po_id, po_avg.average 
              FROM postt 
              JOIN po_avg ON postt.po_id = po_avg.PO";

$avg_result = mysqli_query($conn, $avg_query);

// Store the results in an associative array
$po_avg_map = [];
while ($avg_row = mysqli_fetch_assoc($avg_result)) {
    $po_avg_map[$avg_row['po_id']] = $avg_row['average'];
}
...
while ($row = mysqli_fetch_assoc($sqla)) {
    $po_id = $row['po_id'];
    $desc = $row['desc'];
    $average = isset($po_avg_map[$po_id]) ? number_format($po_avg_map[$po_id], 2) : "N/A"; // Check if average exists
